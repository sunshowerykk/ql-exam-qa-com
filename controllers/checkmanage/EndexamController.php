...
class EndexamController extends BaseController
{
...
//    }
    ...
    public function actionIndex()
    {
        ...
        $view = Yii::$app->view->params['data']="6"; //给框架加参数
    ...
    public function actionFindhead($p='1'){    //查询动作
        ...
        $cDateTime1 = strtotime($req->post('qDateTime1',""));
        $cDateTime2 = strtotime($req->post('qDateTime2',""));

        $RoleID=Yii::$app->user->identity->RoleID;
       // file_put_contents('E:/log/l' . time() . '.txt', print_r($RoleID, true), FILE_APPEND);
        $userid=Yii::$app->user->identity->id;
        $where = " where 1=1 and a.ehgrade=2 and ehgardestatus=2  ";
        if(!empty($cTuserName)){
            $cTuserName=trim($cTuserName); /*去除变量所以空格*/
            $where .=" and a.ehcheckuser = '$cTuserName'";
        }
        if (!empty($cDateTime1) && !empty($cDateTime2)) {
            $where .= " and a.ehgardeendtime between '$cDateTime1' and '$cDateTime2'";
        }
        if (!empty($cDateTime1) && empty($cDateTime2)) {
            $where .= " and a.ehgardeendtime = '$cDateTime1'";
        }
        if (empty($cDateTime1) && !empty($cDateTime2)) {
            $where .= " and a.ehgardeendtime = '$cDateTime2'";
        }
        $perNumber=10; //每页显示的记录数
        if($RoleID==1){
            $sql = "SELECT count(1) FROM sys_examhistory a LEFT JOIN sys_exam b ON a.ehexamid=b.examid  $where";
        }elseif ($RoleID==2 || $RoleID==4){
    ...
    public function actionDelhead(){    //删除 ajax提交自己加参数判断
        ...
        $tra1 = Yii::$app->db->beginTransaction();
        try{
            Yii::$app->db->createCommand()->delete('sys_examhistory', ['ehid' => $c1])->execute();
            $tra1->commit();
        }catch (Exception $e) {
            $tra1->rollBack();
            pub::wrlog($e->getMessage());
